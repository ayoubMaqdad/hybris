 ...
 *
 * This test file tests and demonstrates the behavior of the StadiumService's methods getAllStadium, getStadium
 and
 * saveStadium.
 *
 * We already have a separate file for testing the Stadium DAO, and we do not want this test to implicitly test
 that in
 * addition to the StadiumService. This test therefore mocks out the Stadium DAO leaving us to test the Service
 in
 * isolation, whose behavior should be simply to wraps calls to the DAO: forward calls to it, and passing on
 the results
 * it receives from it.
 */
@UnitTest
public class DefaultStadiumServiceUnitTest
{
    ...
    /**
     * This test tests and demonstrates that the Service's getAllStadiums method calls the DAOs'
     getStadiums method and
    ...
    public void testGetAllStadiums()
    {
        ...
        // Now we make the call to StadiumService's getStadiums() which we expect to call the DAOs'
        //findStadiums() method
        final List<StadiumModel> result = stadiumService.getStadiums();
        // We then verify that the results returned from the service match those returned by the mockedout DAO
    ...
    {
        // Tell Mockito we expect a call to the DAO's getStadium(), and, if it occurs, Mockito should
        //return StadiumModel instance
        when(stadiumDAO.findStadiumsByCode(STADIUM_NAME)).thenReturn(Collections.singletonList
                (stadiumModel));
        // We make the call to the Service's getStadiumForCode() which we expect to call the DAO's
        //findStadiumsByCode()
        final StadiumModel result = stadiumService.getStadiumForCode(STADIUM_NAME);
        // We then verify that the result returned from the Service is the same as that returned from
        //the DAO
