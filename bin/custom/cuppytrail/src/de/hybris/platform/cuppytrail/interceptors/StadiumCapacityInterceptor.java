...
public class StadiumCapacityInterceptor implements ValidateInterceptor, PrepareInterceptor
{
    ...
    public void onPrepare(final Object model, final InterceptorContext ctx) throws InterceptorException
    {
        if (model instanceof StadiumModel)
        {
            ...
            {
                eventService.publishEvent(new CapacityEvent(stadium.getCode(), stadium.
                        getCapacity()));
            }
        }
    }
    ...
    {
        final Integer capacity = stadium.getCapacity();
        if (capacity != null && capacity.intValue() >= BIG_STADIUM && capacity.intValue() <
                TOO_BIG_STADIUM)
        {
            ...
            {
                final Integer oldValue = getItemModelContext(stadium).getOriginalValue
                        (StadiumModel.CAPACITY);
