...
public class DefaultStadiumDAO implements StadiumDAO {
    /**
     We use hybris' FlexibleSearchService for running queries against the database * @see "https://wiki.hybris.com/display/release5/FlexibleSearch"
     */
    @Autowired
...
// Return the list of StadiumModels.
    ...
    @Override public List<StadiumModel> findStadiumsByCode ( final String code){
        final String queryString = "SELECT {p:" + StadiumModel.PK + "}" +
                "FROM {" + StadiumModel._TYPECODE + " AS p} " +
                "WHERE " +
                "{p:" + StadiumModel.CODE + "}=?code ";
        final FlexibleSearchQuery query = new FlexibleSearchQuery(queryString);
